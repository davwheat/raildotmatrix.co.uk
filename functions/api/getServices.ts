import { AssociationCategory } from '../../src/api-types/get-services-types'
import type {
  AssociatedServiceDetail,
  AssociatedServiceLocation,
  Association,
  CancelLatenessReason,
  EndPointLocation,
  NrccMessage,
  StaffServicesResponse,
  TimingLocation,
  TrainService,
} from '../../src/api-types/get-services-types'
...
async function processService(env: Env, service: TrainService): Promise<void> {
  const serviceData = await getServiceByRid(env.RDM_LDBSVWS_GetServiceDetailsByRID_API_KEY, service.rid, false)
  ...
  for (const l in service.subsequentLocations) {
    ...
    for (const a in location.associations) {
      ...
      if (association.category === AssociationCategory.Divide) {
        // Divides only
        association.service = await getServiceByRid(env.RDM_LDBSVWS_GetServiceDetailsByRID_API_KEY, association.rid, false)
      } else if (
        association.category === AssociationCategory.LinkedTo &&
