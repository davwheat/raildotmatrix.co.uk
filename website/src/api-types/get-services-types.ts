// The shapes the /api/getServices worker returns. They live here, rather than in the worker itself, so the site can
// use them without pulling @cloudflare/workers-types into the browser build.
...
export interface CancelLatenessReason {
  tiploc: string
  near: boolean
  value: number
  stationName?: string | null
  /** Darwin spells the reason code with a capital V. It is copied into `value` and removed before the response is returned. */
  Value?: number
}
