...
`led-board/internal/font/fonts/infotec-platform.yaff`, which omits the shared blank top row of the original capture. A two-dot gap separates the
box from the service information. Known websocket coach counts produce a train formation with a filled cab stepping sideways once every two rows
and rounded corners only at the rear end. The cab extends ahead of the first coach, leaving every hollow coach body the same width.
