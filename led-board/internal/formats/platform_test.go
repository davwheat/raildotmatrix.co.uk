...
func TestPlatformBoxRequiresOneRequestedPlatform(t *testing.T) {
	for _, format := range Names {
		for _, platforms := range [][]string{nil, {""}, {" "}, {"2"}, {"1", "2"}, {"2", "2"}} {
			...
			for i, enabled := range []bool{false, true} {
				...
				// Every train is on platform 2. Only the requested filter should enable the box.
			...
			wantBox := format == "infotec" && len(platforms) == 1 && platforms[0] == "2"
